...
def login():
    ...
    try:
        ...
        print(f"id found: {user['id']}")
        print(f"String User ID: {str(user['id'])}")
        access_token = create_access_token(identity="user" + str(user['id']))
        return jsonify({
    ...
    try:
        ...
        if transaction_type == 'transfer':
            ...
            if not destination_account:
                conn.rollback()
                return jsonify({'error': 'Destination account not found or not authorized'}), 404
    ...
    try:
        ...
        if transaction['type'] == 'income':
            ...
            if transaction['source_balance'] < transaction['amount']:
                conn.rollback()
                return jsonify({'error': 'Cannot delete transaction: would result in negative balance'}), 400
            ...
            if transaction['dest_balance'] < transaction['amount']:
                conn.rollback()
                return jsonify({'error': 'Cannot delete transaction: would result in negative balance in destination account'}), 400
...
def get_dashboard_data():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, name, balance FROM accounts WHERE user_id = %s",
            (user_id,)
        )
        accounts = cursor.fetchall()
